...
export const JobsSingleCard = ({ check }) => {
  ...
  let jobData = localStorage.setItem("jobTitle", single.title);

  const navigate = useNavigate();

  const singleData = () => {
    axios
      .get(`https://foremost-bronzed-galley.glitch.me/jobdata/${check}`)
  ...
  return (
    <Box sx={{ padding: "16px" }} className={`${check ? "" : "block"}`}>
      <Box
        sx={{
          boxShadow: "gba(0, 0, 0, 0.15) 0px 3px 3px 0px",
          borderBottom: 1,
          borderColor: "divider",
          paddingBottom: "20px",
        }}
